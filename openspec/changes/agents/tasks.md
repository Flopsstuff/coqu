...
- [x] 4.6 Implement `DELETE /api/agents/:id` — delete agent, kill active install if running, npm uninstall -g, remove env dir
...
- [x] 7.1 Implement `GET /api/agents/:id/env` — read `$HOME/.coqu/agents/<id>/.env`, return empty string if not exists
- [x] 7.2 Implement `PUT /api/agents/:id/env` — write content to env file, create directory if needed
...
- [x] 10.2 Add env file editor (textarea + save button), load via `GET /api/agents/:id/env`, save via `PUT /api/agents/:id/env`
- [x] 10.3 Add polling during `installing` status (every 3s until installed/error)
- [x] 10.4 Add "Reinstall" button calling `POST /api/agents/:id/install`
- [x] 10.5 Add "Delete" button with confirmation

## 11. Frontend — Navigation

- [x] 11.1 Add "Agents" link to Header component
- [x] 11.2 Add routes `/agents`, `/agents/new`, `/agents/:id` to `App.tsx`
