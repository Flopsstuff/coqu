...
- Глобальная страница env-переменных (общий `.env` для всех агентов)
...
Таблица `Agent` без FK на `Project`. Поля: `id`, `name`, `type` (enum: `claude-code`), `status` (enum: `pending`, `installing`, `installed`, `error`), `statusMessage`, `version`, `createdAt`, `updatedAt`.
...
### 4. Глобальный env-файл на volume

Единый env-файл по пути `$HOME/.coqu/.env`, общий для всех агентов. API предоставляет эндпоинты `GET/PUT /api/env` для чтения/записи. Фронтенд показывает отдельную страницу `/env` с текстовым редактором. Файл персистится через volume `home_data` → `$HOME`.

**Альтернатива**: Per-agent env-файлы (`$HOME/.coqu/agents/<id>/.env`). Отвергнуто — все агенты используют одни и те же ключи (ANTHROPIC_API_KEY и т.д.), дублирование не нужно.
...
Новый named volume `home_data` в `docker-compose.yml`, монтируется в `$HOME` (`/root` для alpine). Персистит: данные агентов (`~/.claude/`), глобальный env-файл (`~/.coqu/.env`), любые другие данные home-директории.
...
| GET | `/api/env` | Прочитать глобальный env-файл |
| PUT | `/api/env` | Записать глобальный env-файл |
...
- `/agents/:id` — детали агента (AgentDetailPage)
- `/env` — глобальный редактор env-переменных (EnvPage)

Ссылки в навигации (Header): "Agents" и "Environment". Паттерн аналогичен проектам: список → карточка → деталь.
